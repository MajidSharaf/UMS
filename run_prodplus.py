#!/usr/bin/env python3
"""Run the 5 'production-plus-one-idea' hybrid prompts - production's real,
full prompt text with exactly one idea from a top-scoring short variant
added on top - through project_brief, then executive_summary on each
resulting brief. Produces 5 genuinely deployable candidate briefs (not
narrow probes) and their executive summaries, for direct comparison
against the plain production baseline.

Usage:
    tmux new -s prodplus
    python run_prodplus.py --run-id prodplus
"""
...
OUT_DIR = Path(__file__).resolve().parent / "experiments" / "prodplus"

VARIANTS = ["prodplus-cot3", "prodplus-strict2", "prodplus-risk1", "prodplus-complete2", "prodplus-risk2"]
...
def main():
    ...
    print(f"Plan: {len(VARIANTS)} hybrid prompts x (project_brief + executive_summary), "
    ...
    log("running shared baseline image analysis (production prompt, all images) - reused by every hybrid")
    started = time.time()
    ...
    out_path = run_dir / "prodplus_results.json"
