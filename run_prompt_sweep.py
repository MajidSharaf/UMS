...
def sweep_stage(stage, run_fn, run_args, log, out):
...
def main():
    ...
    if "image_analysis" in args.stages:
        ...
        sweep_stage("image_analysis", pipe.run_image_analysis, ([rep_image], args.vision_model, log), log, out)
    ...
    if "project_brief" in args.stages:
        ...
        baseline_image_results = pipe.run_image_analysis(images, args.vision_model, log)
        sweep_stage("project_brief", pipe.run_project_brief,
                    (pages, baseline_image_results, args.text_model, log), log, out)
    ...
    if "slide_plan" in args.stages:
        ...
        baseline_brief = pipe.run_project_brief(pages, baseline_image_results, args.text_model, log)
        baseline_summary = pipe.run_executive_summary(baseline_brief, args.text_model, log)
        baseline_normalized = pipe.run_normalized_context(baseline_brief, baseline_summary, args.presentation_model, log)
        sweep_stage("slide_plan", pipe.run_slide_plan, (baseline_normalized, args.presentation_model, log), log, out)
    ...
    if "slide_content" in args.stages:
        ...
        baseline_plan = pipe.run_slide_plan(baseline_normalized, args.presentation_model, log)
        if baseline_plan and baseline_plan.get("slides"):
            rep_slide = baseline_plan["slides"][0]
            enrichment = pipe.run_slide_enrichment(rep_slide, baseline_normalized, args.presentation_model, log)
            sweep_stage("slide_content", pipe.run_slide_content,
                        (rep_slide, enrichment, args.presentation_model, log), log, out)
        else:
            log("slide_content: baseline slide plan failed, skipping this stage's sweep")

    summary_path = OUT_DIR / "sweep_summary.json"
    summary_path.write_text(json.dumps(
        {s: {"variants_tested": len(v)} for s, v in out.items()}, indent=2), encoding="utf-8")
    print(f"\nAll done. Per-stage results in {OUT_DIR}/<stage>_sweep.json")
